...
export default function SellerCompleted() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get('/seller/completed-auctions', { params: { user_id: userId } })
        setItems(res.data)
      } catch (e: any) {
        setError(e?.response?.data?.detail || 'Failed to load completed auctions')
      } finally {
        setLoading(false)
      }
    })()
  ...
  if (loading) return <Loader text="Loading completed auctions..." />
  if (error) return <ErrorCard title="Error Loading Completed Auctions" error={error} />

  return (
    <div className="space-y-6">
      <Header title="Completed Auctions" subtitle="Finalized results with winners and profits" icon="✅" />
        ...
        {items.map((a: any) => (
          <div key={a.auction_id} className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-green-700 font-semibold">₹{a.final_price}</div>
            </div>
            <div className="text-sm text-gray-500">Winner: {a.winner_id} • Profit: ₹{a.profit}</div>
          </div>
        ))}
      </div>
      ...
      {items.length === 0 && (
        <div className="text-center py-12">
          ...
          <h4 className="text-xl font-semibold text-gray-800 mb-2">No completed auctions</h4>
